...
static lv_color_t s_flush_scratch[LVGL_PORT_FLUSH_SCRATCH_PIXELS];

void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    LCD *lcd = (LCD *)drv->user_data;
    const int ox = LVGL_PORT_DISP_OFFSET_X_EFFECTIVE;
    const int oy = LVGL_PORT_DISP_OFFSET_Y_EFFECTIVE;
    ...
    const int hor = drv->hor_res;
    const int ver = drv->ver_res;
    ...
    /* Panel-Bus: x/y oft 2-Pixel-alignment (SH8601), sonst drawBitmap-Warnung / kein Bild */
    {
        ...
        if (x_align > 1) {
            const int rem = dst_x % x_align;
            if (rem != 0) {
                const int add = x_align - rem;
                dst_x += add;
                src_x0 += add;
                dst_w -= add;
                if (dst_w <= 0) {
        ...
        if (y_align > 1) {
            const int rem = dst_y % y_align;
            if (rem != 0) {
                const int add = y_align - rem;
                dst_y += add;
                src_y0 += add;
                dst_h -= add;
                if (dst_h <= 0) {
    ...
    const bool need_pack =
        (src_x0 != 0) || (src_y0 != 0) || (dst_w != w) || (dst_h != h);

    bool ok = false;
    const bool is_rgb = (lcd->getBus()->getBasicAttributes().type == ESP_PANEL_BUS_TYPE_RGB);

    if (!need_pack) {
        ok = lcd->drawBitmap(dst_x, dst_y, dst_w, dst_h, (const uint8_t *)color_map);
    } else {
        const uint32_t pixel_count = (uint32_t)dst_w * (uint32_t)dst_h;
        if (pixel_count > LVGL_PORT_FLUSH_SCRATCH_PIXELS) {
            ESP_UTILS_LOGW(
                "flush scratch too small (%u > %u), drop flush region", (unsigned)pixel_count,
                (unsigned)LVGL_PORT_FLUSH_SCRATCH_PIXELS
            );
            lv_disp_flush_ready(drv);
            return;
        }
        lv_color_t *pack = s_flush_scratch;
        const size_t row_bytes = (size_t)dst_w * sizeof(lv_color_t);
        for (int row = 0; row < dst_h; row++) {
            const lv_color_t *src_row = color_map + (src_y0 + row) * w + src_x0;
            memcpy(pack + (ptrdiff_t)row * dst_w, src_row, row_bytes);
        }
        ok = lcd->drawBitmap(dst_x, dst_y, dst_w, dst_h, (const uint8_t *)pack);
    }

    if (is_rgb) {
        lv_disp_flush_ready(drv);
    } else if (!ok) {
        /* drawBitmap kann false liefern (z. B. Grenzen) – ohne Callback sonst kein lv_disp_flush_ready */
        lv_disp_flush_ready(drv);
    }
}
...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (read_touch_result > 0) {
        ...
        lv_coord_t x = (lv_coord_t)point.x - (lv_coord_t)LVGL_PORT_DISP_OFFSET_X_EFFECTIVE;
        lv_coord_t y = (lv_coord_t)point.y - (lv_coord_t)LVGL_PORT_DISP_OFFSET_Y_EFFECTIVE;
        if (x < 0) {
            x = 0;
        } else if (x >= hor) {
            x = hor - 1;
